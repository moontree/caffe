...
// reshape, all bottom need to be the same shape
template <typename Dtype>
...
// forward cpu gemm, convolution process
template <typename Dtype>
void BaseConvolutionLayer<Dtype>::forward_cpu_gemm(const Dtype* input,
    const Dtype* weights, Dtype* output, bool skip_im2col) {
  const Dtype* col_buff = input;
  if (!is_1x1_) {
    if (!skip_im2col) {// im2col
      conv_im2col_cpu(input, col_buffer_.mutable_cpu_data());
    }
    col_buff = col_buffer_.cpu_data();
  }
  for (int g = 0; g < group_; ++g) { // matrix multiply
    // output = 1.0 * weights * col_buf + 0.0 * output 
...
// forward cpu bias, output + bias
template <typename Dtype>
void BaseConvolutionLayer<Dtype>::forward_cpu_bias(Dtype* output,
    const Dtype* bias) {
  // output = 1.0 * bias * bias_multiplier + 1.0 * output 
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_output_,
      out_spatial_dim_, 1, (Dtype)1., bias, bias_multiplier_.cpu_data(),
      (Dtype)1., output);
}

//backward cpu gemm, to previous layer?, return is input
template <typename Dtype>
void BaseConvolutionLayer<Dtype>::backward_cpu_gemm(const Dtype* output,
  ...
  for (int g = 0; g < group_; ++g) {
     // col_buff = 1.0 * weights^T * output + 0.0 * col_buff 
...
// backward weight gemm, update weights
template <typename Dtype>
void BaseConvolutionLayer<Dtype>::weight_cpu_gemm(const Dtype* input,
  ...
  for (int g = 0; g < group_; ++g) {
    // weights = 1.0 * output * col_buff^T + 1.0 * weights 
...
void BaseConvolutionLayer<Dtype>::backward_cpu_bias(Dtype* bias,
    const Dtype* input) {// 1 * input * bias_multiplier + 1 * bias
